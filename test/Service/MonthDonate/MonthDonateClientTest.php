<?php


namespace Yeepay\Yop\Sdk\Service\MonthDonate;


use PHPUnit\Framework\TestCase;
use Yeepay\Yop\Sdk\Config\AppSdkConfig;
use Yeepay\Yop\Sdk\Exception\YopClientException;
use Yeepay\Yop\Sdk\Service\MonthDonate\Model as Model;

class MonthDonateClientTest extends TestCase
{

    private static $client;

    public static function setUpBeforeClass()
    {
        // 常用以下两种方式构造client，根据自身情况选择其一
        // 1. 基于本地配置文件方式：支持绝对路径、相对路径(相对于根目录)，适用于将文件打包到项目中，一起部署
        $sdkConfigFile = "yop_sdk_config_default.json";
        self::$client = MonthDonateClientBuilder::builder($sdkConfigFile)->build();

        // 2. 基于json串方式：适用于自定义配置来源，从其他存储加载(比如说数据库、配置中心等)
        //$sdkConfigJson = '{"app_key":"app_xxx","server_root":"https://openapi.yeepay.com/yop-center","yos_server_root":"https://yos.yeepay.com/yop-center","yop_public_key":[{"store_type":"string","cert_type":"RSA2048","value":""}],"isv_private_key":[{"store_type":"string","cert_type":"RSA2048","value":""}],"encrypt_key":"AWXI5fZGXmTFVHfjyzBMMrrk1IF9dCqGJt3JIZrKY90=","http_client":{"connect_timeout":3000,"read_timeout":30000,"max_conn_total":200,"max_conn_per_route":100}}';
        //self::$client = MonthDonateClientBuilder::builder(new AppSdkConfig(\GuzzleHttp\json_decode($sdkConfigJson, true)))->build();
    }

    /**
     * 修改月捐(/rest/v1.0/month-donate/change)
     *
     * @test
     */
    public function changeTest()
    {
        $request = new Model\ChangeRequest();

        $request->setBody(new \Yeepay\Yop\Sdk\Service\MonthDonate\Model\ChangeMonthDonateRequestDTO());
        try{
            $response = self::$client->change($request);
            print_r($response->getResult());
        }catch (\Exception $e) {
            echo 'Exception when calling MonthDonateClient->change: ', $e->getMessage(), PHP_EOL;
        }
    }
    /**
     * 关闭月捐(/rest/v1.0/month-donate/close)
     *
     * @test
     */
    public function closeTest()
    {
        $request = new Model\CloseRequest();

        $request->setBody(new \Yeepay\Yop\Sdk\Service\MonthDonate\Model\CloseMonthDonateRequestDTO());
        try{
            $response = self::$client->close($request);
            print_r($response->getResult());
        }catch (\Exception $e) {
            echo 'Exception when calling MonthDonateClient->close: ', $e->getMessage(), PHP_EOL;
        }
    }
    /**
     * 创建易宝公益账号(/rest/v1.0/month-donate/create-user)
     *
     * @test
     */
    public function createUserTest()
    {
        $request = new Model\CreateUserRequest();

        $request->setBody(new \Yeepay\Yop\Sdk\Service\MonthDonate\Model\CreateUserRequestDTO());
        try{
            $response = self::$client->createUser($request);
            print_r($response->getResult());
        }catch (\Exception $e) {
            echo 'Exception when calling MonthDonateClient->createUser: ', $e->getMessage(), PHP_EOL;
        }
    }
    /**
     * 开通月捐(/rest/v1.0/month-donate/open)
     *
     * @test
     */
    public function openTest()
    {
        $request = new Model\OpenRequest();

        $request->setBody(new \Yeepay\Yop\Sdk\Service\MonthDonate\Model\OpenMonthDonateRequestDTO());
        try{
            $response = self::$client->open($request);
            print_r($response->getResult());
        }catch (\Exception $e) {
            echo 'Exception when calling MonthDonateClient->open: ', $e->getMessage(), PHP_EOL;
        }
    }
    /**
     * 查询月捐订单(/rest/v1.0/month-donate/query-order-info)
     *
     * @test
     */
    public function queryOrderInfoTest()
    {
        $request = new Model\QueryOrderInfoRequest();

        $request->setBody(new \Yeepay\Yop\Sdk\Service\MonthDonate\Model\QueryMonthDonateOrderRequestDTO());
        try{
            $response = self::$client->queryOrderInfo($request);
            print_r($response->getResult());
        }catch (\Exception $e) {
            echo 'Exception when calling MonthDonateClient->queryOrderInfo: ', $e->getMessage(), PHP_EOL;
        }
    }
    /**
     * 查询签约信息(/rest/v1.0/month-donate/query-sign-info)
     *
     * @test
     */
    public function querySignInfoTest()
    {
        $request = new Model\QuerySignInfoRequest();

        $request->setBody(new \Yeepay\Yop\Sdk\Service\MonthDonate\Model\QuerySignRequestDTO());
        try{
            $response = self::$client->querySignInfo($request);
            print_r($response->getResult());
        }catch (\Exception $e) {
            echo 'Exception when calling MonthDonateClient->querySignInfo: ', $e->getMessage(), PHP_EOL;
        }
    }
    /**
     * 查询用户月捐信息(/rest/v1.0/month-donate/query-user-info)
     *
     * @test
     */
    public function queryUserInfoTest()
    {
        $request = new Model\QueryUserInfoRequest();

        $request->setBody(new \Yeepay\Yop\Sdk\Service\MonthDonate\Model\QueryMonthDonateInfoRequestDTO());
        try{
            $response = self::$client->queryUserInfo($request);
            print_r($response->getResult());
        }catch (\Exception $e) {
            echo 'Exception when calling MonthDonateClient->queryUserInfo: ', $e->getMessage(), PHP_EOL;
        }
    }
}
