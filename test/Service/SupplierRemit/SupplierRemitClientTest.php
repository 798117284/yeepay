...
class SupplierRemitClientTest extends TestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        self::$client = SupplierRemitClientBuilder::builder($sdkConfigFile)->build();
        ...
        //self::$client = SupplierRemitClientBuilder::builder(new AppSdkConfig(\GuzzleHttp\json_decode($sdkConfigJson, true)))->build();
    }

    /**
     * 付款对账文件获取接口(/yos/v1.0/supplier-remit/bill/download)
     *
     * @test
     */
     ...
     * @test
     */
    public function bill_download_v1_0Test()
    {
        $request = new Model\BillDownloadV10Request();

        $request->setRemitDate("remitDate_example");
        try{
            $response = self::$client->bill_download_v1_0($request);
            $response->save('targetFilePath');
        }catch (\Exception $e) {
            echo 'Exception when calling SupplierRemitClient->bill_download_v1_0: ', $e->getMessage(), PHP_EOL;
        }
    }
}
