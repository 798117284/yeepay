...
class ApplyV10RequestMarshaller implements RequestMarshaller
{
    /**
     * @var ApplyV10RequestMarshaller
     */
    private static $instance;

    public static function __init()
    {
        self::$instance = new ApplyV10RequestMarshaller();
    }

    /**
     * @return ApplyV10RequestMarshaller
     */
    ...
    public function marshal($request)
    {
        ...
        if($request->getIsUnfreezeResidualAmount() != null){
            $internalRequest->addParameter('isUnfreezeResidualAmount', ObjectSerializer::sanitizeForSerialization($request->getIsUnfreezeResidualAmount(), 'string'));
        }
        if($request->getDivideAmount() != null){
            $internalRequest->addParameter('divideAmount', ObjectSerializer::sanitizeForSerialization($request->getDivideAmount(), 'string'));
        }
        if($request->getDivideRule() != null){
            $internalRequest->addParameter('divideRule', ObjectSerializer::sanitizeForSerialization($request->getDivideRule(), 'string'));
        }
...
ApplyV10RequestMarshaller::__init();
