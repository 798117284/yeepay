<?php


namespace Yeepay\Yop\Sdk\Service\HotelTravel;


use PHPUnit\Framework\TestCase;
use Yeepay\Yop\Sdk\Config\AppSdkConfig;
use Yeepay\Yop\Sdk\Exception\YopClientException;
use Yeepay\Yop\Sdk\Service\HotelTravel\Model as Model;

class HotelTravelClientTest extends TestCase
{

    private static $client;

    public static function setUpBeforeClass()
    {
        // 常用以下两种方式构造client，根据自身情况选择其一
        // 1. 基于本地配置文件方式：支持绝对路径、相对路径(相对于根目录)，适用于将文件打包到项目中，一起部署
        $sdkConfigFile = "yop_sdk_config_default.json";
        self::$client = HotelTravelClientBuilder::builder($sdkConfigFile)->build();

        // 2. 基于json串方式：适用于自定义配置来源，从其他存储加载(比如说数据库、配置中心等)
        //$sdkConfigJson = '{"app_key":"app_xxx","server_root":"https://openapi.yeepay.com/yop-center","yos_server_root":"https://yos.yeepay.com/yop-center","yop_public_key":[{"store_type":"string","cert_type":"RSA2048","value":""}],"isv_private_key":[{"store_type":"string","cert_type":"RSA2048","value":""}],"encrypt_key":"AWXI5fZGXmTFVHfjyzBMMrrk1IF9dCqGJt3JIZrKY90=","http_client":{"connect_timeout":3000,"read_timeout":30000,"max_conn_total":200,"max_conn_per_route":100}}';
        //self::$client = HotelTravelClientBuilder::builder(new AppSdkConfig(\GuzzleHttp\json_decode($sdkConfigJson, true)))->build();
    }

    /**
     * 订单核验结果同步(/rest/v1.0/hotel-travel/order/verify-sync)
     *
     * @test
     */
    public function orderVerifySyncTest()
    {
        $request = new Model\OrderVerifySyncRequest();

        $request->setBody(new \Yeepay\Yop\Sdk\Service\HotelTravel\Model\OrderInfoCheckRequestDto());
        try{
            $response = self::$client->orderVerifySync($request);
            print_r($response->getResult());
        }catch (\Exception $e) {
            echo 'Exception when calling HotelTravelClient->orderVerifySync: ', $e->getMessage(), PHP_EOL;
        }
    }
    /**
     * 订单核验结果查询(/rest/v1.0/hotel-travel/query/order-result)
     *
     * @test
     */
    public function order_result_queryTest()
    {
        $request = new Model\OrderResultQueryRequest();

        $request->setBody(new \Yeepay\Yop\Sdk\Service\HotelTravel\Model\OrderCheckQueryRequestDto());
        try{
            $response = self::$client->order_result_query($request);
            print_r($response->getResult());
        }catch (\Exception $e) {
            echo 'Exception when calling HotelTravelClient->order_result_query: ', $e->getMessage(), PHP_EOL;
        }
    }
    /**
     * 订单核验结果同步(/rest/v1.0/hotel-travel/order/verify-sync)
     *
     * @test
     */
    public function order_syncTest()
    {
        $request = new Model\OrderSyncRequest();

        $request->setBody(new \Yeepay\Yop\Sdk\Service\HotelTravel\Model\OrderInfoCheckRequestDto());
        try{
            $response = self::$client->order_sync($request);
            print_r($response->getResult());
        }catch (\Exception $e) {
            echo 'Exception when calling HotelTravelClient->order_sync: ', $e->getMessage(), PHP_EOL;
        }
    }
    /**
     * 订单核验结果查询(/rest/v1.0/hotel-travel/query/order-result)
     *
     * @test
     */
    public function queryOrderResultTest()
    {
        $request = new Model\QueryOrderResultRequest();

        $request->setBody(new \Yeepay\Yop\Sdk\Service\HotelTravel\Model\OrderCheckQueryRequestDto());
        try{
            $response = self::$client->queryOrderResult($request);
            print_r($response->getResult());
        }catch (\Exception $e) {
            echo 'Exception when calling HotelTravelClient->queryOrderResult: ', $e->getMessage(), PHP_EOL;
        }
    }
}
